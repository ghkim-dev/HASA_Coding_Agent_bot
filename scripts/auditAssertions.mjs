...
// 계측을 치운다. 시작할 때 TRACE 만 지우고 끝나고는 아무것도 치우지 않아서,
// 매 실행마다 684KB 짜리 로그와 shim 세 개가 작업 디렉터리에 남았다. 여기까지
// 왔다는 것은 trace 를 이미 다 읽었다는 뜻이므로 넷 다 지워도 된다.
//
// `assert` 를 가로채는 shim 이 남아 있는 것이 특히 나쁘다 — 다음에 무언가가
// 그것을 집어 들면 이 감사와 상관없는 실행이 계측된 채로 돌아간다.
for (const path of [TRACE, SHIM, RESOLVER, HOOK]) rmSync(path, { force: true });

...
const MUTATIONS = [
  ...
  // requirementMemory — 지난 요구와 그 결과. 여기가 무너지면 사용자의 정정이라는
  // 공짜 정답을 잃거나, 비교 불가능한 벡터를 이웃으로 세게 된다.
  ["M312", "정정을 결과로 읽지 않음", "src/design/requirementMemory.ts",
    '  if (spec.supersededBy !== undefined) return "superseded";\n', "", "memory"],
  ["M313", "하네스가 스스로 붙인 요구까지 기억함", "src/design/requirementMemory.ts",
    '    if (spec.status === "system_added") continue;\n', "", "memory"],
  ["M314", "공간 없이도 벡터를 담음", "src/design/requirementMemory.ts",
    "      ...(vector !== undefined && space !== undefined ? { vector, space } : {}),",
    "      ...(vector !== undefined ? { vector, space } : {}),", "memory"],
  ["M315", "정정을 되돌릴 수 있게 함", "src/design/requirementMemory.ts",
    "  return OUTCOME_RANK[outcome] > OUTCOME_RANK[row.outcome] ? { ...row, outcome } : row;",
    "  return { ...row, outcome };", "memory"],
  ["M316", "다른 벡터 공간의 행도 이웃으로 셈", "src/design/requirementMemory.ts",
    "    if (row.vector === undefined || row.space !== space) continue;",
    "    if (row.vector === undefined) continue;", "memory"],
  ["M317", "같은 유사도의 순서를 정하지 않음", "src/design/requirementMemory.ts",
    "    .sort((a, b) => b.similarity - a.similarity || a.row.id.localeCompare(b.row.id))",
    "    .sort((a, b) => b.similarity - a.similarity)", "memory"],
  ...
  ["M173", "범위 조사를 다시 위치로 읽음", "src/design/functionalExtract.ts",
    ...
    "      ? token.replace(/(?:안에서만|에서만|에서|안에|에만|까지|부터|[안밖위속앞뒤옆]의)$/u, \"\")", "recall"],
  ...
  // ---- C4.17: what cannot be a target ---------------------------------------
  ["M202", "의존명사를 대상으로 삼음", "src/design/functionalExtract.ts",
  ...
  // ---- C4.33: a bound noun or a time word wearing a particle --------------------
  ["M264", "조사를 떼지 않고 어휘를 봄 — `걸로`·`중에는` 이 대상이 됨", "src/design/functionalExtract.ts",
    "    const head = out.replace(/(?:에는|에도|에서|에게|에|으로|로|만|도|은|는|이|가)+$/u, \"\");",
    "    const head = out;", "recall"],
  ["M265", "조사를 걸친 의존명사를 다시 놓침", "src/design/functionalExtract.ts",
    "        BOUND_NOUN.test(head) ||",
    "        false ||", "recall"],
  ["M266", "시점명사를 부사처럼 건너뜀 — 전처리를 학습하게 됨", "src/design/functionalExtract.ts",
    "      closesPhrase: TIME_HEAD.test(head),",
    "      closesPhrase: false,", "recall"],
  ["M267", "목록을 세는 수사를 대상에 넣음 — `mp4랑 gif 둘`", "src/design/functionalExtract.ts",
    "  /^(?:그것|이것|저것|그거|이거|저거|그|이|저|좀|다|전부|모두|잘|적당히|알아서|한번|다시|또|이번|이번에|이번에는|안에서만|여기서|거기서|말고|그리고|또한|하지만|그런데|및|등|수|있는|있을|없는|않는|않은|않을|되는|할|하는|해서|해|그대로|반드시|가능하면|절대|꼭|제대로|계속|미리|먼저|우선|전혀|아직|실제|실제로|맞춰|맞추어|따라|위해|통해|대해|같이|함께|오늘|어제|내일|왜|어떻게|무엇|뭐|뭔가|무언가|누군가|언젠가|어디|언제|누가|얼마나|어떤|어느|무슨|게|것|건|걸|거|바|줄|뿐|때문|따름|아니|아니라|것이|말이|하나|둘|셋|넷)$/;",
    "  /^(?:그것|이것|저것|그거|이거|저거|그|이|저|좀|다|전부|모두|잘|적당히|알아서|한번|다시|또|이번|이번에|이번에는|안에서만|여기서|거기서|말고|그리고|또한|하지만|그런데|및|등|수|있는|있을|없는|않는|않은|않을|되는|할|하는|해서|해|그대로|반드시|가능하면|절대|꼭|제대로|계속|미리|먼저|우선|전혀|아직|실제|실제로|맞춰|맞추어|따라|위해|통해|대해|같이|함께|오늘|어제|내일|왜|어떻게|무엇|뭐|뭔가|무언가|누군가|언젠가|어디|언제|누가|얼마나|어떤|어느|무슨|게|것|건|걸|거|바|줄|뿐|때문|따름|아니|아니라|것이|말이)$/;", "recall"],
  // ---- C4.34: an adverb standing in front of the object -------------------------
  ["M268", "부사를 다시 목적어에 붙임 — `없이 영상`, `특히 화질`", "src/design/functionalExtract.ts",
    "        ADVERB.test(out) ||",
    "        false ||", "recall"],
  ["M269", "생산적인 부사 형태를 읽지 않음 — `-히`·`-게` 가 목적어가 됨", "src/design/functionalExtract.ts",
    ...
    "  /^(?:없이|일단|대충|되도록|가급적|웬만하면|빨리|얼른|이미|방금|아까|이제|그냥|차라리|오히려|아무튼|어쨌든|따로|바로|직접)$/u;", "recall"],
  ...
  // ---- C4.38: a retirement that leaves no mark, and a ban the design forgets ---
  ["M281", "정정이 표시를 남기지 않고 문장을 바꿔치움 — 물러난 것과 안 읽힌 것이 같아짐", "src/design/requirementSpec.ts",
    "    return contradicted ? { ...spec, supersededBy: turnId } : spec;",
    "    return contradicted ? { ...spec, text: `${spec.text} (dropped)` } : spec;", "conversation"],
  ["M282", "설계가 읽은 금지를 마지막 턴까지 들고 있지 않음", "src/design/requirementSpec.ts",
    "      id: `${input.turnId}-forbid-${klass}`,",
    "      id: `${input.turnId}-forbidden-${klass}`,", "conversation"],
  // ---- C4.39: a topic read as a modifier, and a particle doubled after Latin --
  ["M283", "주제 표지를 단 명사구를 맨 수식어로 버림 — `API Key는 SecretStorage에만 저장하고` 가 통째로 사라짐", "src/design/functionalExtract.ts",
    "  if (!runMarked && !runTopicMarked && trailing?.grammar === true && trailing.carriesNoun) {",
    "  if (!runMarked && trailing?.grammar === true && trailing.carriesNoun) {", "recall"],
  ["M284", "주제 표지를 읽지 않음", "src/design/functionalExtract.ts",
    "      carriesTopicMark: /[은는]$/u.test(token.replace(/,$/u, \"\")) && bare.length > 0,",
    "      carriesTopicMark: false,", "recall"],
  ["M285", "라틴 낱말 뒤의 조사 겹침을 놓침 — `SecretStorage에를`", "src/design/functionalExtract.ts",
    "  if (/(?:(?<=[가-힣]{2,})|(?<=[A-Za-z0-9]))(?:에서|에게|으로|에)$/u.test(object)) return \"\";",
    "  if (/(?<=[가-힣]{2,})(?:에서|에게|으로|에)$/u.test(object)) return \"\";", "recall"],
...
// A search string carrying a carriage return can never match.
//
// The loop below normalises CRLF away before it searches — it has to, or a
// multi-line anchor written with bare newlines would silently match nothing on
// a Windows checkout. The mirror of that is this: an anchor captured *from* a
// CRLF file keeps its CR and then matches nothing for the opposite reason.
//
// It happened, to three entries at once, and the way it happened is worth
// recording: the anchors were generated by reading the source and splitting on
// "\n", which is the obvious thing to do and leaves a CR on every line. The
// harness reported them as unapplied rather than as passing, which is the
// behaviour it was built for — but a script written beside it checked them with
// a raw `includes` and disagreed, so the report looked like the wrong one. The
// check belongs here, where an anchor cannot be written wrong in the first
// place.
const withCarriageReturn = MUTATIONS.filter(([, , , from, to]) => `${from}${to}`.includes("\r"));

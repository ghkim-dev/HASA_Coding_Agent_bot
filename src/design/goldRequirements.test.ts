 ...
 * ## 집계 하나 옆에 사례 하나씩
 *
 * 집계 핀은 그대로 남는다. `{ hit: 65, of: 65 }` 은 분모를 주장하는 문장이고,
...
describe("요구사항 정확성 — 분모를 함께", () => {
  test("recall 61/61", () => {
    assert.deepEqual(score.requirementRecall, { hit: 65, of: 65, value: 1 });
  ...
  test("precision 59/59 — 발명이 0이다", () => {
    ...
    assert.deepEqual(score.requirementPrecision, { hit: 65, of: 65, value: 1 });
    assert.deepEqual(score.spurious, [], "요청에 없는 요구사항을 만들었습니다");
  });

  test("target 정확도 61/61, span 근거 61/61", () => {
    assert.deepEqual(score.targetAccuracy, { hit: 65, of: 65, value: 1 });
    assert.deepEqual(score.spanGrounding, { hit: 65, of: 65, value: 1 });
  });
  ...
  test("relation 47/48", () => {
    ...
    // 실행한다"] either way.
    assert.deepEqual(score.relationAccuracy, { hit: 49, of: 50, value: 0.98 });
  });
...
describe("질문 정확성 — 분모를 함께", () => {
  ...
  test("질문 상한을 넘는 사례가 없다", () => {
    assert.deepEqual(score.questionCeiling, { hit: 45, of: 45, value: 1 });
  });
...
describe("Startable 과 Executable 은 다른 주장이다", () => {
  test("Requirement Startability 43/43", () => {
    ...
    assert.deepEqual(score.requirementStartability, { hit: 45, of: 45, value: 1 });
  });
  ...
  test("Harness Executability 43/43", () => {
    ...
    assert.deepEqual(score.harnessExecutability, { hit: 45, of: 45, value: 1 });
  });

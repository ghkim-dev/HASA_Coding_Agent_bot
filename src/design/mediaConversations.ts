...
export const MEDIA_CONVERSATIONS: readonly MediaConversation[] = [
  ...
  },
  {
    ...
    standing: ["모델을 비교한다"],
    // "이미지와 프롬프트로 동영상을 생성한다" — 첫 턴은 방법을 말하고, 추출기는
    // 이제 그 방법을 문장에 되돌려 놓는다(`mannerBefore`). 이 못은 그 전에
    // 쓰였고 맨머리만 적고 있었다.
    //
    // 물러난 사실을 부정으로만 확인하는 동안에는 이 어긋남이 보이지 않았다 —
    // "이 문장은 서 있지 않다" 는 런타임이 그 문장을 아예 만들지 않아도 참이기
    // 때문이다. 정정 축이 "물러난 자리에 있어야 한다" 를 함께 보게 되자마자
    // 죽은 못으로 드러났다. 이 파일 머리가 경고한 그대로다: 렌더링이 옮겨가면
    // 못 박은 문장이 따라가야 한다.
    superseded: ["이미지와 프롬프트로 동영상을 생성한다"],
  },
  {
  ...
  },
  {
    ...
    // "비교한다", not "요청한 내용을 살펴본다". 비교 와 살펴봄 은 다른 결과물이고,
    // 목적어가 있을 때는 이미 "결과를 비교한다" 로 나오고 있었다. 목적어가
    // 없다는 이유만으로 사용자가 쓴 동사를 바꿔 부르던 쪽이 틀렸다.
    standing: ["비교한다"],

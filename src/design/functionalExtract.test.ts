 ...
 *
 * 한 문장이 하나의 test 다. 이름은 `<문장> · <축>` 이므로, 실패한 줄 하나가 어떤
 * 문장의 어떤 축이 어긋났는지를 그대로 말한다. 주장을 묶어 두던 곳은 describe 가
 * 되었고, 그 주장이 왜 그것인지 적어 둔 주석은 그 describe 의 문서주석이다.
 *
 * 말뭉치를 만드는 `before()` 는 여기에 없고, 두지 않는다. `node --test` 는 `before()`
 * 훅이 throw 하면 그 아래 테스트를 **cancelled** 로 처리하면서 요약줄에는 `fail 0` 을
 * 찍고, describe 본문에서 throw 하면 그 아래 테스트가 세어지지도 않은 채 사라진다 —
 * 요약줄은 역시 `fail 0` 이다. 둘 다 이 파일의 사본으로 재어 봤다 — 훅 하나를 터뜨리면
 * `cancelled 9 / fail 0`, 같은 값을 describe 본문에서 읽게 하면 테스트가 217 개에서
 * 210 개로 줄고 사라진 7 개는 실패로도 취소로도 세어지지 않는다. 요약줄이 거짓말을 하는
 * 자리가 둘이라는 뜻이므로, 추출기를 부르는 일은 전부 test 안에서 일어난다. 추출기가
 * 통째로 터진 사본은 `cancelled 0 / fail 211` 이었다 — 사례마다 제 이름으로 실패한다.
 */
...
/**
 * 한 문장을 축마다 다시 읽지 않기 위한 것.
 *
 * 값이 아니라 함수를 상수로 두는 이유는 파일 머리에 적은 것과 같다 — describe 본문에서
 * 추출기를 부르면, 그것이 터졌을 때 그 아래 테스트가 요약줄에 세어지지도 않은 채
 * 사라진다. 첫 test 가 부르고, 나머지는 그 결과를 본다.
 */
const once = (text: string): (() => FunctionalCandidate[]) => {
  let read: FunctionalCandidate[] | undefined;
  return () => (read ??= of(text));
};

describe("동사와 목적어", () => {
  test("로그인 오류를 수정해줘. · 동사+목적어", () => {
   ...
   * `보다` — 본동사일 때만.
   *
   * `보여줘` 는 읽히고 `봐줘` 는 읽히지 않아서 "로그를 봐줘" 가 아무 요구사항도
   * 만들지 않았다. 고치면서 조사를 요구하게 했는데, 그 조건이 무엇을 막는지
   * 재는 것이 여기 아래쪽 절반이다. `보다` 는 한국어의 시도 보조동사이기도
   * 해서, 조사 없이 `봐` 를 읽으면 "테스트를 실행해 봐줘" 가 `execute` 옆에
   * 없던 `inspect` 를 하나 더 얻는다 — 사용자가 하지 않은 요청이 계획에 서는
   * 것이고, 이 파일의 다른 어떤 test 도 그것을 보지 못했다.
   */
  describe("봐줘 는 본동사일 때만 읽는다", () => {
    test("로그를 봐줘 · 살펴본다", () => {
      assert.deepEqual(texts("로그를 봐줘."), ["로그를 살펴본다"]);
    });

    test("저장소 코드만 봐줘 · 살펴본다", () => {
      assert.deepEqual(texts("저장소 코드만 봐줘."), ["저장소 코드를 살펴본다"]);
    });

    test("설정 파일을 봐주세요 · 띄어쓰기 없는 형태", () => {
      assert.deepEqual(texts("설정 파일을 봐주세요."), ["설정 파일을 살펴본다"]);
    });

    test("테스트를 실행해 봐줘 · 실행 하나뿐이다", () => {
      assert.deepEqual(texts("테스트를 실행해 봐줘."), ["테스트를 실행한다"]);
    });

    test("로그인 오류를 고쳐 봐줘 · 수정 하나뿐이다", () => {
      assert.deepEqual(texts("로그인 오류를 고쳐 봐줘."), ["로그인 오류를 수정한다"]);
    });

    test("확인해 봐줘 · 검증 하나뿐이다", () => {
      assert.deepEqual(texts("확인해 봐줘."), ["테스트를 실행해 결과를 확인한다"]);
    });

    /**
     * 조사 요구가 실제로 무엇을 막는지 재는 자리.
     *
     * 위의 세 문장은 이 조건이 없어도 통과한다 — 실행·수정·검증이 이미 이기고
     * 있어서 `봐` 가 무엇을 하든 답이 같기 때문이다. 변이를 걸어 보고 알았다.
     *
     * 구별이 되는 것은 아는 동사가 하나도 없는 문장이다. 조사를 요구하지 않으면
     * 보조동사 앞의 어미가 목적어가 되어 **"돌려를 살펴본다"** 가 나온다 —
     * 사용자가 하지 않은 요청이고, 그 목적어는 낱말도 아니다.
     */
    test("아는 동사가 없으면 어미를 목적어로 삼지 않는다", () => {
      assert.deepEqual(texts("한번 돌려 봐줘."), []);
      assert.deepEqual(texts("만들어 봐줘."), []);
    });
  });
   ...
   * now agree.
   */
  describe("목적어 생략이 허용되는 테스트·실행·설명", () => {
    test("테스트해줘 · 문장", () => {
  ...
  test("테스트해줘 · 생략된 목적어는 unresolved 로 기록되고 지어내지 않는다", () => {
  ...
  test("로그인 오류를 수정해줘. · 목적어가 있으면 binding 이 resolved", () => {
...
describe("부정문", () => {
  describe("부정문에서 긍정 요구사항을 만들지 않는다", () => {
    for (const text of [
      "파일을 수정하지 마.",
      "파일을 수정하진 마.",
      "코드를 수정하지는 말아줘.",
  ...
  describe("`-지 말고` 뒤의 긍정 요청은 살린다", () => {
    test("수정하지 말고 설명만 해줘. · 문장", () => {
      assert.deepEqual(texts("수정하지 말고 설명만 해줘."), ["설명한다"]);
    });

    test("실행하지 말고 코드만 보여줘. · 문장", () => {
      assert.deepEqual(texts("실행하지 말고 코드만 보여줘."), ["코드를 살펴본다"]);
    });
  });

  test("테스트는 실행하지 말고 결과만 설명해줘. · `말고` 는 목적어가 되지 않는다", () => {
  ...
  test("테스트는 실행하지 말고 결과만 설명해줘. · 금지 절은 긍정 요청의 span 에 들어가지 않는다", () => {
...
describe("과거 실패 보고", () => {
  describe("과거의 실패는 요청이 아니다", () => {
    test("아까는 실행하지 못했어. · 요구사항 없음", () => {
      assert.deepEqual(of("아까는 실행하지 못했어."), []);
    });

    test("테스트가 실패했어. · 요구사항 없음", () => {
      assert.deepEqual(of("테스트가 실패했어."), []);
    });
  });

  test("아까는 실행하지 못했어. 원인을 고치고 이번에는 다시 실행해줘. · 실패 보고 뒤의 요청은 읽는다", () => {
...
describe("병렬 요청", () => {
  test("로그인 오류를 수정하고 테스트해줘. · `-고` 로 이어진 두 요청을 둘 다 읽는다", () => {
  ...
  test("main.py 코드도 보여주고 실제 실행 결과도 보여줘. · `하고` 가 아닌 `-고` 도 절 경계다", () => {
  ...
  test("기존 Arena와 Worktree를 유지하면서 HASA Coding Agent를 추가해줘. · `면서` 로 이어진 두 요청", () => {
...
describe("조사와 파일명", () => {
  test("main.py 코드만 분석해줘. · 점이 든 파일명을 문장 경계로 자르지 않는다", () => {
    assert.deepEqual(texts("main.py 코드만 분석해줘."), ["main.py 코드를 분석한다"]);
  });

  test("실제 실행 결과도 보여줘. · `도` 는 목적어에 남지 않는다", () => {
  ...
  /**
   * `-도` 를 떼는 두 관문, 각각 자기 문장으로.
   *
   * 지우는 조건은 두 개다: 지우고도 두 음절이 남을 것, 그리고 `MEASURE_NOUN` 에
   * 없을 것. 말뭉치에는 앞엣것만 지나는 문장이 있었고 뒤엣것을 지나는 문장은
   * `해상도` 하나뿐이었다 — 그래서 `MEASURE_NOUN` 에서 `속도` 를 지워도 저장소
   * 전체가 초록이었다. 죽은 항목이어서가 아니라, 그 항목이 발화하는 토큰을 아무도
   * 쓰지 않아서였다: `속도` 는 지우면 `속` 한 음절만 남아 길이 관문에서 먼저
   * 걸리고, `MEASURE_NOUN` 이 실제로 붙드는 것은 `가속도` 처럼 두 음절이 남는
   * 쪽이다.
   */
  describe("`-도` 를 지우는 두 관문", () => {
    test("응답 속도를 개선해줘. · 길이 관문", () => {
      // "속도" is a word, not a noun plus the additive particle. 여기서 지키는
      // 것은 길이다 — `속` 은 한 음절이라 지워지지 않는다.
      assert.deepEqual(texts("응답 속도를 개선해줘."), ["응답 속도를 개선한다"]);
    });

    test("가속도 확인해줘. · 낱말 목록 관문", () => {
      // 지우면 `가속` 이 남아 길이 관문을 통과해 버린다. 이 토큰을 붙드는 것은
      // `MEASURE_NOUN` 의 `속도` 항목뿐이고, 그것을 지우면 사용자가 말하지 않은
      // `가속` 이 대상이 된다.
      assert.deepEqual(texts("가속도 확인해줘."), ["가속도를 확인한다"]);
    });

    test("해상도 확인해줘. · 같은 관문의 다른 낱말", () => {
      assert.deepEqual(texts("해상도 확인해줘."), ["해상도를 확인한다"]);
    });

    test("결과도 보여줘. · 목록에 없으면 지운다", () => {
      // 반대 방향. `결과도` 는 명사에 조사가 붙은 것이므로 지워야 한다.
      assert.deepEqual(texts("결과도 보여줘."), ["결과를 살펴본다"]);
    });
  });

  describe("`만` / `에서만` / `안에서만`", () => {
    test("auth 폴더 안에서만 수정해줘. · 문장", () => {
...
describe("나열과 범위는 하나의 목적어다", () => {
  describe("접속 조사로 이어진 목록은 통째로 남는다", () => {
    test("CNN과 ViT로 분류기를 만들고 각각 학습해줘. · 문장", () => {
    ...
    test("개와 고양이 분류 프로젝트를 만들어줘. · 문장", () => {
      assert.deepEqual(texts("개와 고양이 분류 프로젝트를 만들어줘."), [
        "개와 고양이 분류 프로젝트를 만든다",
      ]);
    });

    test("웹과 Hugging Face를 참고해줘. · 문장", () => {
      assert.deepEqual(texts("웹과 Hugging Face를 참고해줘."), ["웹과 Hugging Face를 참고한다"]);
    });
  });

  test("CNN부터 Transformer까지 사용해줘. · 범위 조사 부터·까지는 위치가 아니라 목적어의 일부다", () => {
   ...
   */
  describe("목록이 아닌 구는 여전히 두 토큰에서 끊긴다", () => {
    test("src 폴더 안에서만 로그를 추가해줘. · 접속 조사가 없으면 창은 그대로다", () => {
      assert.deepEqual(texts("src 폴더 안에서만 로그를 추가해줘."), ["로그를 추가한다"]);
    });

    test("사용할 수 있는 모델을 확인해줘. · 창이 끊은 대상", () => {
  ...
  /**
   * `과` 로 끝나는 명사와 접속 조사 `과` 는 뒤에 올 말이 있는지로 갈린다.
   *
   ...
   */
  describe("`과` 로 끝나는 명사와 접속 조사 `과`", () => {
    test("이전 실행 결과를 비교해줘. · 뒤에 올 말이 없으면 명사의 끝", () => {
      assert.deepEqual(texts("이전 실행 결과를 비교해줘."), ["실행 결과를 비교한다"]);
    });

    test("지난 학습 결과와 로그를 비교해줘. · 뒤에 올 말이 있으면 접속 조사", () => {
...
describe("사용자가 쓴 동사가 요구사항에 남는다", () => {
  describe("분류가 아니라 그 동사로 읽힌다", () => {
    test("README를 한국어로 번역해줘. · 문장", () => {
   ...
   */
  describe("분류의 말이 더 정확한 곳에서는 분류의 말을 쓴다", () => {
    test("기존 동작을 유지해줘. · 문장", () => {
      assert.deepEqual(texts("기존 동작을 유지해줘."), ["기존 동작을 그대로 유지한다"]);
    });

    test("로그인 오류를 fix 해줘. · 문장", () => {
      assert.deepEqual(texts("로그인 오류를 fix 해줘."), ["로그인 오류를 수정한다"]);
    });
  });

  test("결과와 로그를 살펴봐줘. · 살펴보다는 출력하는 말이면서 읽는 말이기도 하다", () => {
...
describe("무엇이 대상이고 무엇이 대상이 아닌가", () => {
   ...
   */
  describe("도구격 `-로`/`-으로` 는 방법이지 대상이 아니다", () => {
    test("이미지를 동영상으로 만들어줘. · 문장", () => {
      assert.deepEqual(texts("이미지를 동영상으로 만들어줘."), ["이미지를 동영상으로 만든다"]);
    });

    test("이미지를 동영상으로 만들어줘. · 대상", () => {
      assert.equal(of("이미지를 동영상으로 만들어줘.")[0]?.object, "이미지");
    });

    test("설정을 기본값으로 되돌려줘. · 문장", () => {
   ...
   * `동영상으`.
   */
  describe("목적어 표시가 없으면 `-로` 구가 유일한 명사다", () => {
    test("동영상으로 만들어줘. · 문장", () => {
   ...
   */
  describe("숫자로 끝나는 대상에 조사를 올바르게 붙인다", () => {
    test("mp4로 내보내줘. · 문장", () => {
      assert.deepEqual(texts("mp4로 내보내줘."), ["mp4를 내보낸다"]);
    });

    test("결과 영상을 mp4로 내보내줘. · 문장", () => {
   ...
   */
  describe("`-도` 로 끝나는 측정 명사에서 조사를 떼지 않는다", () => {
    test("해상도를 설정해줘. · 문장", () => {
  ...
  test("프레임 수와 해상도를 설정할 수 있게 해줘. · `-ㄹ 수 있게 해줘` 는 기능 요청이다", () => {
  ...
  test("실패하면 다시 시도하게 해줘. · 사용자가 쓴 동사", () => {
  ...
  test("프롬프트를 바꾸면 결과가 어떻게 달라지는지 비교해줘. · 조건절의 동사는 요청이 아니고, 대상도 아니다", () => {
  ...
  test("이미지를 업로드하면 5초짜리 영상으로 변환해줘. · 대상이 조건절 뒤에 있으면 수식어를 대상으로 삼지 않는다", () => {
...
describe("의존명사와 부정칭은 대상이 아니다", () => {
   ...
   */
  describe("`걸`, `것`, `거` 는 목적어가 되지 않는다", () => {
    test("뭔가 좋은 걸 만들어줘. · 요구사항 없음", () => {
      assert.deepEqual(texts("뭔가 좋은 걸 만들어줘."), []);
    });

    test("좋은 것 좀 추가해줘. · 요구사항 없음", () => {
   ...
   */
  describe("부정칭 대명사는 대상이 아니다", () => {
    test("뭔가를 추가해줘. · 요구사항 없음", () => {
      assert.deepEqual(texts("뭔가를 추가해줘."), []);
    });

    test("무언가를 만들어줘. · 요구사항 없음", () => {
      assert.deepEqual(texts("무언가를 만들어줘."), []);
    });
  });

  test("빠른 걸 만들어줘. · 의존명사가 빠지면 남은 관형어만으로 대상을 만들지 않는다", () => {
  ...
  describe("명사가 살아 있으면 관형어는 그대로 남는다", () => {
    test("파일 등을 추가해줘. · 문장", () => {
...
describe("정정문은 요청이 아니다", () => {
  test("아니, 실행하라는 게 아니라 코드 결과물을 보여달라는 말이야. · `-라는 게 아니라` 는 앞의 동사를 부정한다", () => {
...
describe("경동사 하나에 묶인 여러 행위", () => {
  ...
  test("전처리와 학습을 해줘. · 이름을 댈 수 없는 것과 묶여 있어도 목적어로 삼지 않는다", () => {
   ...
   */
  describe("과·와 로 끝나는 두 음절 낱말은 쪼개지 않는다", () => {
    test("결과 확인을 해줘. · 문장", () => {
  ...
  test("전처리 후 학습을 해줘. · 시점을 말하는 구는 대상이 아니다", () => {
  ...
  test("기존 결과와 비교해줘. · `와` 가 붙은 비교 상대는 대상이 아니다", () => {
   ...
   */
  describe("혼자 선 경동사도 사용자가 쓴 낱말로 읽는다", () => {
    test("학습까지 해줘. · 문장", () => {
    ...
    test("테스트를 해줘. · 부류 문구가 그 낱말을 담고 있으면 그대로 둔다", () => {
    ...
    test("번역을 해줘. · 목적어 없는 `modify` 는 요구사항이 아니다", () => {
...
describe("`-는지` 로 물은 것은 대상이 아니지만 주제다", () => {
  describe("사물을 부르지 않은 절도 무엇에 대한 것인지 말한다", () => {
    test("실제로 호출되는지도 알려줘. · 문장", () => {
      assert.deepEqual(texts("실제로 호출되는지도 알려줘."), ["실제로 호출되는지 살펴본다"]);
    });

    test("무엇이 문제인지만 알려줘. · 문장", () => {
      assert.deepEqual(texts("무엇이 문제인지만 알려줘."), ["무엇이 문제인지 살펴본다"]);
    });
  });

  test("프롬프트를 바꾸면 결과가 어떻게 달라지는지 비교해줘. · 절이 제 목적어를 부르면 그쪽이 이긴다", () => {
  ...
  test("어디가 느린지 알려주고, 왜 실패하는지도 알려줘. · 두 개의 물음은 두 개의 요청이다", () => {
...
describe("동사 없는 조각은 뒤 절에 붙는다", () => {
  test("웹과 Hugging Face, HASA도 참고하고, 결과를 비교해줘. · 쉼표로 끊긴 목록은 하나의 목적어다", () => {
  ...
  test("먼저, 코드를 고쳐줘. · 붙여 넣은 부사는 목적어가 되지 않는다", () => {
...
describe("`-어서` 는 뒤 절이 제 대상을 부를 때만 경계다", () => {
  test("모델 목록을 확인해서 후보를 비교해줘. · 뒤 절이 목적어를 부르면 두 요청으로 읽는다", () => {
...
describe("조사를 걸친 의존명사·시점명사", () => {
  test("이미지를 10장 뽑아서 그중 제일 나은 걸로 영상을 만들어줘. · 의존명사에 조사가 붙어도 대상이 아니다", () => {
    assert.deepEqual(texts("이미지를 10장 뽑아서 그중 제일 나은 걸로 영상을 만들어줘."), [
      "영상을 만든다",
    ]);
  });

  describe("시점명사 앞의 명사는 그 시점의 것이다", () => {
    test("전처리 후 학습을 해줘. · 문장", () => {
      assert.deepEqual(texts("전처리 후 학습을 해줘."), ["학습한다"]);
    });

    test("생성 중에는 진행률을 보여줘. · 문장", () => {
      assert.deepEqual(texts("생성 중에는 진행률을 보여줘."), ["진행률을 살펴본다"]);
    });

    test("배포 이후에 로그를 확인해줘. · 문장", () => {
   ...
   * 지운다.
   */
  describe("명사를 꾸미는 시점 낱말은 그 명사의 일부다", () => {
    test("이전 결과를 비교해줘. · 문장", () => {
  ...
  describe("목록을 세는 수사는 대상이 아니다", () => {
    test("결과물은 mp4랑 gif 둘 다 저장해줘. · 문장", () => {
      assert.deepEqual(texts("결과물은 mp4랑 gif 둘 다 저장해줘."), [
        "결과물은 mp4랑 gif를 저장한다",
      ]);
    });

    test("이미지 한 장을 영상으로 만들어줘. · 대상", () => {
...
describe("읽지 않기로 한 것들", () => {
   ...
   */
  describe("`쓰다` 는 읽지 않는다 — 쓰기와 사용하기를 가릴 수 없다", () => {
    test("CNN과 Transformer를 쓰고 학습까지 해줘. · 쓰기 절은 아무것도 내놓지 않는다", () => {
      const used = texts("CNN과 Transformer를 쓰고 학습까지 해줘.");
      assert.deepEqual(used, ["학습한다"]);
    });

    test("보고서를 쓰고 공유해줘. · 요구사항 없음", () => {
      assert.deepEqual(texts("보고서를 쓰고 공유해줘."), []);
    });
  });

  test("실패하면 해상도를 낮춰서 다시 시도해줘. · `-어서` 가 줄어든 형태는 절 경계로 읽지 못한다", () => {
  ...
  test("영상 길이는 5초를 넘기지 않게 해줘. · `-지 않게 해줘` 로 말한 제약은 읽지 못한다", () => {
  ...
  test("사용자가 프롬프트를 입력하면 영상이 나오게 해줘. · `-게 해줘` 가 하다를 거치지 않으면 읽지 못한다", () => {
   ...
   */
  describe("명사를 잇는 `하고` 는 동사 어미와 구별되지 않는다", () => {
    test("좋은 오픈소스 모델하고 HASA 모델도 추가해줘. · 문장", () => {
      assert.deepEqual(texts("좋은 오픈소스 모델하고 HASA 모델도 추가해줘."), [
        "HASA 모델을 추가한다",
      ]);
    });

    test("전처리하고 학습해줘. · `전처리하고` 는 대상이 되지 않는다", () => {
      const got = texts("전처리하고 학습해줘.");
      assert.ok(!got.some((t) => t.includes("전처리하고")), got.join(", "));
    });
  });

  test("실제로 호출되는지도 알려줘. · `실제로` 는 목적어가 아니다", () => {
...
describe("목적어 구는 어디서 끊기는가", () => {
  describe("위치를 나타내는 말은 목적어에 붙지 않는다", () => {
    test("src 폴더 안에서만 로그를 추가해줘. · 위치를 나타내는 말", () => {
      assert.deepEqual(texts("src 폴더 안에서만 로그를 추가해줘."), ["로그를 추가한다"]);
    });

    test("CI에서 pytest를 실행해줘. · 문장", () => {
      assert.deepEqual(texts("CI에서 pytest를 실행해줘."), ["pytest를 실행한다"]);
    });

    /**
     * `안의` — 처소 명사에 관형격이 붙은 형태.
     *
     * "8_09 폴더 안의 main.py를 실행해줘" 는 **안의 main.py를 실행한다** 였다.
     * 대상이 의존명사로 시작하고 폴더를 하나도 부르지 않는데, 시나리오 말뭉치의
     * 키워드는 main.py 와 실행이 살아남아 2/2 를 그대로 찍고 있었다 — 집계가
     * 초록인 채로 지어낸 대상이 사람에게 보이던 자리다.
     *
     * 두 문장인 이유: 처소 표지를 떼는 규칙은 문장이 목적어를 `을/를` 로
     * 표시했는지에 따라 서로 다른 가지를 탄다. 표시된 가지만 덮으면 표시되지
     * 않은 가지는 아무 말뭉치도 지나가지 않는 죽은 방어선이 된다.
     */
    test("8_09 폴더 안의 main.py를 실행해줘. · 목적어가 표시된 절", () => {
      assert.deepEqual(texts("8_09 폴더 안의 main.py를 실행해줘."), ["main.py를 실행한다"]);
    });

    test("src 폴더 안의 로그만 보여줘. · 목적어가 표시되지 않은 절", () => {
      assert.deepEqual(texts("src 폴더 안의 로그만 보여줘."), ["로그를 살펴본다"]);
    });

    /**
     * 주제 표지를 단 명사구는 맨 수식어가 아니다.
     *
     * "API Key는 SecretStorage에만 저장하고 설정 파일에는 쓰지 마." 는
     * **아무것도** 내놓지 않았다. 비밀을 어디에 둘지에 대한 요구사항이다.
     *
     * 세 가지가 한 문장에 겹쳐 있었고, 셋 다 이 블록이 지킨다:
     *
     *   · `에만` 이 처소 표지 목록에 없어서 `SecretStorage에만` 이 목적어 창에
     *     남았다.
     *   · 조사 겹침 가드가 한글 두 음절을 요구해서 라틴 낱말 뒤에서는 걸리지
     *     않았고, 그래서 한때 **Key는 SecretStorage에를 저장한다** 였다.
     *   · 그것들을 고치자 이번에는 맨 수식어 규칙이 `API Key는` 을 통째로
     *     버렸다 — 그 규칙의 관문이 "절이 `을/를` 로 목적어를 표시했는가" 뿐인데
     *     이 문장은 주제로 표시하기 때문이다.
     *
     * 이 사례가 미리보기 fixture 에만 있고 추출기 말뭉치에는 없었다는 것이
     * 셋이 함께 숨어 있을 수 있었던 이유다.
     */
    test("API Key는 SecretStorage에만 저장해줘. · 주제로 표시된 대상", () => {
      const got = texts("API Key는 SecretStorage에만 저장해줘.");
      assert.deepEqual(got, ["API Key를 저장한다"]);
      assert.ok(!got.some((t) => t.includes("에를")), got.join(", "));
    });

    test("API Key는 SecretStorage에만 저장하고 설정 파일에는 쓰지 마. · 금지가 뒤따라도", () => {
      // `쓰다` 는 읽지 않으므로 뒤 절은 아무것도 내지 않는다. 앞 절은 낸다.
      assert.deepEqual(texts("API Key는 SecretStorage에만 저장하고 설정 파일에는 쓰지 마."), [
        "API Key를 저장한다",
      ]);
    });

    test("로그를 파일에만 저장해줘. · 목적어가 표시된 절의 `에만`", () => {
      assert.deepEqual(texts("로그를 파일에만 저장해줘."), ["로그를 저장한다"]);
    });

    /**
     * 라틴 낱말 뒤의 조사 겹침.
     *
     * 겹침 가드는 한글 앞에서 두 음절을 요구한다 — `결과`·`성과` 처럼 그 음절로
     * 끝나는 낱말을 지키기 위해서다. 라틴 낱말은 한글 음절 `에` 로 끝날 수
     * 없으므로 그 조건이 필요 없는데, 조건이 문자 종류를 가리지 않아 라틴 뒤에서는
     * 가드가 걸리지 않았다.
     *
     * `도` 로 표시된 절이라야 이 자리에 닿는다. `을/를` 이 있으면 처소 표지가
     * 먼저 떨어져 나가서 `에` 가 남지 않는다.
     */
    test("로그도 S3에 저장해줘. · 라틴 낱말 뒤에는 조사를 더 붙이지 않는다", () => {
      const got = texts("로그도 S3에 저장해줘.");
      assert.deepEqual(got, ["로그 S3에 저장한다"]);
      assert.ok(!got.some((t) => t.includes("에를")), got.join(", "));
    });
  });
   ...
   */
  describe("뒤따르는 부사는 목적어를 끊지 않는다", () => {
    test("auth 폴더 안에서만 수정하고 끝내줘. · 문장", () => {
      assert.deepEqual(texts("auth 폴더 안에서만 수정하고 끝내줘."), ["auth 폴더를 수정한다"]);
    });

    test("기존 동작은 그대로 유지해줘. · 문장", () => {
      assert.deepEqual(texts("기존 동작은 그대로 유지해줘."), ["기존 동작을 그대로 유지한다"]);
    });
  });

  test("테스트가 실패하면 로그를 추가해줘. · 조건절은 목적어에 붙지 않는다", () => {
    assert.deepEqual(texts("테스트가 실패하면 로그를 추가해줘."), ["로그를 추가한다"]);
  });

  test("오늘은 main.py를 실행해줘. · 주제 조사가 붙은 앞말은 다른 구다", () => {
   ...
   */
  describe("때를 나타내는 말은 목적어가 아니다", () => {
    test("오늘 main.py를 실행해줘. · 문장", () => {
...
describe("유지 요청", () => {
  ...
  test("수정하면서 기존 테스트는 그대로 유지해줘. · `그대로 유지` 도 읽는다", () => {
    assert.deepEqual(texts("수정하면서 기존 테스트는 그대로 유지해줘."), ["기존 테스트를 그대로 유지한다"]);
  });
});

describe("설명·분석 요청", () => {
  describe("분석과 설명은 inspect 다", () => {
...
describe("범위 제한과 조건", () => {
  test("auth 폴더 안에서만 수정해줘. · scope 는 원문에서 다시 읽는다", () => {
  ...
  test("기존 클라이언트가 사용 중이라면 API 형식을 변경하지 마. · 조건부 요청은 조건을 남긴다", () => {
...
describe("경계 입력", () => {
  test("빈 문자열 · 아무것도 만들지 않는다", () => {
    assert.deepEqual(of(""), []);
  });

  test("공백뿐인 문자열 · 아무것도 만들지 않는다", () => {
    assert.deepEqual(of("   \n  "), []);
  });

  describe("동사가 없으면 아무것도 만들지 않는다", () => {
    test("적당히 잘 좀 해줘. · 요구사항 없음", () => {
      assert.deepEqual(of("적당히 잘 좀 해줘."), []);
    });

    test("알아서 다 해줘. · 요구사항 없음", () => {
   ...
   */
  describe("영어 명령문은 어순을 바꿔 읽는다", () => {
    const got = once("Fix the login bug and run the tests.");

    test("Fix the login bug and run the tests. · 행위", () => {
      assert.deepEqual(
        got().map((c) => c.action),
        ["modify", "execute"],
      );
    ...
    test("Fix the login bug and run the tests. · 대상", () => {
      assert.deepEqual(
        got().map((c) => c.object),
        ["login bug", "tests"],
      );
   ...
   */
  describe("영어 서술문과 의문문은 명령으로 읽지 않는다", () => {
    for (const text of [
   ...
   */
  describe("영어 금지문은 그 동사를 요구로 만들지 않는다", () => {
    const mixed = once("Do not modify anything, just explain.");

    test("Don't run the tests. · 요구사항 없음", () => {
      assert.deepEqual(of("Don't run the tests."), []);
    });

    test("Do not modify anything, just explain. · 금지된 행위", () => {
      assert.ok(!mixed().some((c) => c.action === "modify"), "the forbidden act was read as a request");
    });

    test("Do not modify anything, just explain. · 요청된 행위", () => {
      assert.ok(mixed().some((c) => c.action === "inspect"), "the act that *was* asked for went missing");
    });
  });

  test("재현해줘. 그리고 검증해줘. · 접속사는 목적어가 되지 않는다", () => {
    const got = texts("재현해줘. 그리고 검증해줘.");
    assert.ok(!got.some((t) => t.includes("그리고")), got.join(", "));
  });

  test("HASA Coding Agent를 추가해줘. · 한국어와 영어 식별자가 섞인 문장", () => {
   ...
   */
  describe("관계절과 부사구는 영어 목적어의 일부가 아니다", () => {
    test("Build a project that turns an image into a video. · 대상", () => {
      assert.equal(of("Build a project that turns an image into a video.")[0]?.object, "project");
    });

    test("Generate an image from text. · 대상", () => {
      assert.equal(of("Generate an image from text.")[0]?.object, "image");
    });

    test("Export the result video as mp4. · 대상", () => {
   ...
   */
  describe("간접 목적어와 보어 표지는 목적어에 남지 않는다", () => {
    test("Show me the result as a preview. · 대상", () => {
      assert.equal(of("Show me the result as a preview.")[0]?.object, "result");
    });

    test("Check for errors in the log. · 대상", () => {
   ...
   */
  describe("대상이 대명사뿐이거나 부사구뿐이면 대상은 없다", () => {
    test("Fix the login error and test it. · 두 번째 절의 대상", () => {
      assert.equal(of("Fix the login error and test it.")[1]?.object, "");
    });

    test("Download the model and run it on the GPU. · 두 번째 절의 대상", () => {
   ...
   */
  describe("영어 `and` 는 두 행위를 잇기도 하고 두 명사를 잇기도 한다", () => {
    test("Support both image generation and video generation. · 대상", () => {
      assert.equal(
    ...
    test("Fix the login error and test it. · 행위", () => {
   ...
   * not read.
   */
  describe("영어 요청에는 영어 문장으로 답한다", () => {
    test("Run it. · 문장", () => {
      assert.deepEqual(texts("Run it."), ["run the requested command"]);
    });

    test("Fix the login error and test it. · 문장", () => {
  ...
  test("Let me set the frame count. · `let me` 도 요청을 여는 말이다", () => {
   ...
   */
  describe("`build a` 는 만드는 것이고 `build` 는 돌리는 것이다", () => {
    test("Build a tool that generates images. · 행위", () => {
      assert.equal(of("Build a tool that generates images.")[0]?.action, "create");
    });

    test("Build the project. · 행위", () => {
   ...
   */
  describe("접속사 하나만 남았을 때 그것은 목적어가 아니다", () => {
    const got = once("Train and evaluate the model.");

    test("Train and evaluate the model. · 문장", () => {
      assert.deepEqual(got().map((c) => c.text), ["train", "evaluate the model"]);
    });

    test("Train and evaluate the model. · 대상", () => {
   ...
   */
  describe("목적어가 없으면 사용자가 쓴 동사로 읽는다", () => {
    test("Compare it with the previous result. · 문장", () => {
      assert.deepEqual(texts("Compare it with the previous result."), ["compare"]);
    });

    test("Don't run it, just explain. · 문장", () => {
      assert.deepEqual(texts("Don't run it, just explain."), ["explain"]);
    });

    test("Run it. · 부류 문구가 그 낱말을 담고 있으면 그대로 둔다", () => {
      assert.deepEqual(texts("Run it."), ["run the requested command"]);
    });
  });
   ...
   */
  describe("`tell` 도 동사이고, 물음은 목적어가 아니다", () => {
    test("Check the model list and tell me which ones are usable. · 문장", () => {
    ...
    test("Tell me whether it is actually called. · 문장", () => {
      assert.deepEqual(texts("Tell me whether it is actually called."), [
        "tell me whether it is actually called",
      ]);
    });

    test("Show me what changes if I change the prompt. · 대상", () => {
      assert.equal(of("Show me what changes if I change the prompt.")[0]?.object, "");
    });
  });

  test("Tell me which model is fastest and tell me why it fails. · 한 턴의 두 물음은 두 요청이다", () => {
   ...
   */
  describe("`make it possible to` 는 기능 요청이다", () => {
    test("Make it possible to set the frame rate and the resolution. · 문장", () => {
      assert.deepEqual(texts("Make it possible to set the frame rate and the resolution."), [
        "set the frame rate and the resolution",
      ]);
    });

    test("Add the login button. · 문장", () => {
   ...
   * 영어는 목록 앞에 동사를 둔다.
   */
  describe("쉼표로 이어진 목록은 하나의 목적어다", () => {
    test("Download the dataset, the weights, and the config. · 문장", () => {
      assert.deepEqual(texts("Download the dataset, the weights, and the config."), [
        "download the dataset, the weights, and the config",
      ]);
    });

    test("Generate an image, a video, and a thumbnail. · 문장", () => {
   ...
   */
  describe("쉼표 뒤에 온 것이 목록이 아니면 목적어가 아니다", () => {
    test("Add a button, and make it blue. · 문장", () => {
      assert.deepEqual(texts("Add a button, and make it blue."), ["add a button"]);
    });

    test("Fix the bug, the tests are failing. · 문장", () => {
      assert.deepEqual(texts("Fix the bug, the tests are failing."), ["fix the bug"]);
    });

    test("Fix the bug, the tests are failing and the build is red. · 문장", () => {
      assert.deepEqual(texts("Fix the bug, the tests are failing and the build is red."), [
        "fix the bug",
      ]);
    });

    test("Check the logs, the error appeared yesterday. · 문장", () => {
...
describe("span 은 UTF-16 좌표로 정확하다", () => {
  test("🔥 로그인 오류를 수정해줘 · 이모지가 있어도 원문을 가리킨다", () => {
  ...
  test("테스트해줘. 테스트해줘. 테스트해줘. · 같은 절이 반복돼도 각 span 이 제자리를 가리킨다", () => {
  ...
  describe("모든 span 이 원문 범위 안에 있다", () => {
    for (const TEXT of [
      "로그인 오류를 수정하고 테스트해줘.",
...
describe("같은 동사가 여러 번 나오는 문장", () => {
  test("로그인 오류를 수정해줘. 그리고 로그인 오류를 다시 수정해줘. · 같은 행동과 같은 대상은 한 번만 기록한다", () => {
    assert.deepEqual(texts("로그인 오류를 수정해줘. 그리고 로그인 오류를 다시 수정해줘."), [
      "로그인 오류를 수정한다",
    ]);
  });

  test("로그인 오류를 수정하고 결제 오류를 수정해줘. · 대상이 다르면 둘 다 기록한다", () => {

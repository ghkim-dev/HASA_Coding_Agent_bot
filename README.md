...
| 방어선이 실제로 지탱하는지 | 변이 **271개**, 예상 밖 무반응 0 | `pnpm design:mutate` |
| 적어 둔 정답이 실제로 검사되는지 | **857/862**, 예상 밖 0 | `node scripts/answers.mjs` |
| 치환 문자열이 코드와 어긋나지 않았는지 | 271/271 | `pnpm design:anchors` |
...
결과물에 대한 금지는 **읽히지만 남지 않는다**. "워터마크는 넣지 말고" 는 요구사항을 만들지
않는 데까지는 옳게 처리되고, 그다음 사라진다 — `statedProhibitions` 가 아는 세 부류는 모두
도구 관문(실행·수정·웹)이고 이것은 결과물에 대한 제약이기 때문이다. 세 가지 미디어 주제에서
가장 흔한 제약이므로, 고치지 않은 채로 이름을 붙여 둔다.
...
pnpm design:mutate   # 방어선 271개를 하나씩 지우고 테스트가 잡는지 (수십 분)
node scripts/answers.mjs  # 적어 둔 정답 862개를 하나씩 바꾸고 테스트가 잡는지 (~10분)
```

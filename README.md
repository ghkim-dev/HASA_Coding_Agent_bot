...
| 무엇을 | 얼마나 | 어디서 |
|---|---|---|
| 사용자가 말한 낱말이 살아남는 비율 | **42/47** | `evalScenarioRecall.test.ts` |
...
| 방어선이 실제로 지탱하는지 | 변이 **224개**, 예상 밖 무반응 0 | `pnpm design:mutate` |
| 치환 문자열이 코드와 어긋나지 않았는지 | 224/224 | `pnpm design:anchors` |
...
pnpm design:mutate   # 방어선 224개를 하나씩 지우고 테스트가 잡는지 (수십 분)
```

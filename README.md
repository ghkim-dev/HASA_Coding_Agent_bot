...
| 방어선이 실제로 지탱하는지 | 변이 **251개**, 예상 밖 무반응 0 | `pnpm design:mutate` |
| 치환 문자열이 코드와 어긋나지 않았는지 | 251/251 | `pnpm design:anchors` |
...
pnpm design:mutate   # 방어선 251개를 하나씩 지우고 테스트가 잡는지 (수십 분)
```

...
| 무엇을 | 얼마나 | 어디서 |
|---|---|---|
| 요구사항 추출 (개발셋) | 43개 사례 | `src/design/goldCases.ts` — 정답을 먼저 쓰고 나중에 돌림 |
| 요구사항 추출 (홀드아웃) | 33개 사례, 해시 고정 | `src/design/holdoutCases.ts` — 구현이 한 번도 맞춰본 적 없음 |
| 사용자가 말한 낱말이 살아남는 비율 | **35/47** | `evalScenarioRecall.test.ts` — 평가기 시나리오 24턴 |
| 모델 추천 정확도 | **14/14** | `recommendationCases.test.ts` — 합성 후보로 만든 상대 비교 |
| 불변식 (지어내지 않음·근거·금지 일관성) | 117턴 / 후보 123개 | `extractInvariants.test.ts` |
| 방어선이 실제로 지탱하는지 | 변이 171개, 예상 밖 무반응 0 | `pnpm design:mutate` |

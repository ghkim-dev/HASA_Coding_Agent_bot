...
|---|---|---|
| 객관 게이트 (빌드·테스트·타입·lint) | **없음** — 실행할 코드가 없다 | G0~G7, hard 게이트 실패 시 즉시 탈락 |
| 숫자 점수 | **없음.** `score`는 항상 `null` | §3의 식으로 계산 |
...
| 값 | 상황 | 왜 유보가 정당한가 |
|---|---|---|
| `null` | 객관 점수 차가 임계값 이상이거나, 게이트 통과 + judge가 AB/BA 일치 | 유보하지 않는다 |
| `never_compared` | 게이트를 통과한 후보가 1개 | 비교가 실제로 일어나지 않았다. "이겼다"가 아니라 "혼자 남았다" |
| `unstable_judge` | 순서를 뒤집자 judge가 판정을 바꿈 | 시스템이 **자기 판정의 신뢰 불가를 실측**했다. 여기서 승자를 만들면 없는 정보를 지어내는 것 |
| `tie` | 가를 근거가 없음 | 동일 |
| `judge_only` | 응답 비교 — 객관 게이트가 존재하지 않는 모드 | 판정이 한 모델의 산문 독해에만 의존한다. 모드의 성질이지 헤징이 아니다 |

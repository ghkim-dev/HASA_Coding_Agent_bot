...
| 값 | 상황 | 왜 유보가 정당한가 |
|---|---|---|
| `null` | 객관 점수 차가 임계값 이상이거나, judge가 AB/BA에서 일치 | 유보하지 않는다 |
...
| `tie` | 가를 근거가 없음 | 동일 |
| `judge_unavailable` | judge가 재시도 후에도 판정 JSON을 내놓지 못함 | 판정이 갈린 것이 아니라 **판정 자체가 없다**. `unstable_judge`와 처방이 반대다 — 이쪽은 예산·모델 교체로 고치고, 저쪽은 증거 추가로만 고친다 |

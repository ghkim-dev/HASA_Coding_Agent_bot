...
| S4 | 위 전부가 못 가릴 때 | judge가 낸 **분별 주장을 실제로 실행한 결과** | 1회 |
...
마지막 계단에서는 "어느 쪽이 나은가"를 묻지 않는다. **두 제출물을 가르는, 기계로 확인 가능한 주장
하나**를 `Check` 형식(§S0과 같은 타입)으로 요구하고, 그 검사를 두 제출물에 직접 실행한다.

판정 근거는 judge의 `better` 필드가 아니라 **실행 결과**다. 세 가지로 갈린다.

| 결과 | 판정 |
|---|---|
| 주장이 한쪽만 통과시키고, 그쪽이 judge가 지목한 쪽 | 그 후보가 승자. 근거는 의견이 아니라 **측정값** |
| 주장이 양쪽에서 같은 결과 | 가르지 못했다 → 사다리 소진 |
| 주장이 한쪽만 통과시키는데 **judge가 지목한 쪽이 아님** | judge가 든 근거가 자기 선택에 대해 거짓이다 → 사다리 소진 |

세 번째가 이 계단의 존재 이유다. **확신에 찬 틀린 근거는 확신에 찬 옳은 근거와 구별되지 않는다.**
반복(S2)도 다른 모델(S3)도 "안정적인 주장"에 점수를 줄 뿐, 안정적인 주장과 참인 주장을 가르지
못한다. 주장을 실행해 보는 것만이 그 둘을 가른다.

한 방향으로만 제시해도 되는 이유: 위치 편향이 `better`를 흔들 수는 있지만, 그것만으로 판정을 끌고 갈
수는 없다. 편향된 `better`도 **그 방향으로 실제로 판별하는 검사**의 뒷받침을 받아야 한다.

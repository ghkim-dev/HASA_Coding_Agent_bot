...
## 11. 모델 피커가 무엇을 보여줘야 하는가 (2026-08-03)

실행 중인 확장의 피커 화면에서 발견됐다. 21개 중 14개를 제공하고 있었는데 그중 2개는 대화가 불가능한 모델이었다.

| 모델 | 카탈로그 | 선택했다면 |
|---|---|---|
| `whisper-large-v3` | `audio`, available, **callable** | `404 Not Found` |
| `tts-ko` | `audio`, **unavailable**, **not callable** | `403 model_not_on_key` |

원인은 두 개이고 서로 다른 종류다.

**(1) `audio`가 대화 가능으로 분류돼 있었다.** `DEDICATED_ENDPOINT`는 image·video·embeddings·rerank만 담고 있었고 audio는 "그 외 전부 → `/v1/chat/completions`"에 속해 있었다. 그런데 `whisper-large-v3`는 카탈로그가 available·callable이라고 말하는데도 chat에서 404다. 음성 모델도 이미지·동영상과 마찬가지로 자기 엔드포인트를 쓴다.

**(2) `available`·`callable`을 아무도 보지 않았다.** 미디어 경로(`byModality`)는 처음부터 두 필드를 확인했지만 대화 경로는 modality만 봤다. `tts-ko`는 게이트웨이가 이미 "실행 중이 아니고 호출할 수 없다"고 말해둔 모델인데도 피커에 올라왔고, 선택하면 사용자가 할 수 있는 게 아무것도 없는 403을 받았다. **한쪽만 확인하던 비대칭 자체가 결함이었다.**

`offerForConversation(entry)`로 세 조건을 한곳에 모았다. 카탈로그에 **없는** 모델은 계속 제공한다 — 피커 목록은 `/v1/models`에서 오고 카탈로그는 주석일 뿐이므로, 한쪽에만 있는 모델이 사라지면 안 되고 포털 API가 죽었을 때 피커가 비어서도 안 된다.

수정 후 라이브 게이트웨이 기준 **12개**가 표시되며, 이는 §9.1이 실제 추론으로 측정한 `response` 자격 12개와 **정확히 일치한다.** 카탈로그 메타데이터와 실측이라는 독립적인 두 경로가 같은 답을 냈다.

---

## 12. 첨부와 분석 — 무엇이 되고 무엇이 안 되는가 (2026-08-03)

### 12.1 문서

의존성 추가 없이 처리한다. Office와 HWPX는 zip+XML이므로 `node:zlib`과 태그 제거로 충분하다.

| 형식 | 상태 | 근거 |
|---|---|---|
| `.docx` / `.docm` | 됨 | `word/document.xml` |
| `.xlsx` / `.xlsm` | 됨 | 공유 문자열 테이블 + 시트, 탭 구분 행 |
| `.pptx` / `.pptm` | 됨 | 실제 문서 2건에서 각각 18,595자·150자 추출 |
| `.hwpx` | 됨 | 실제 문서 2건에서 12,544자·2,499자 추출 |
| `.pdf` | **조건부** | 아래 참조 |
| `.hwp` (구버전) | **안 됨** | OLE 복합 문서 바이너리. hwpx/PDF 변환을 안내한다 |

**PDF는 라틴 문자 본문만 읽힌다.** 실측한 한글 PDF 2건은 글꼴이 자체 CID 인코딩을 써서, 바이트가 글리프 인덱스이고 내장 CMap 없이는 의미가 없다. 읽으려면 글꼴까지 해석하는 파서가 필요하다.

첫 구현은 이 경우를 **통과시켰다** — 압축 스트림의 바이트에서 `(...)`를 긁어모아 24만 자의 바이너리를 텍스트로 반환했고, "글자 20개 이상" 가드는 그것을 막지 못했다. 지금은 FlateDecode를 풀고, `BT`(텍스트 객체 시작)가 있는 스트림만 보고, 결과가 산문처럼 보이는지 비율로 검사한다. **확신에 찬 헛소리를 반환하느니 거부하는 쪽이 낫다.**

### 12.2 음성

`POST /v1/audio/transcriptions` (multipart, OpenAI 형식). 모델은 카탈로그에서 `audio` modality로 찾으므로 코드가 이름을 알지 않는다.

> **운영자 조치:** 이 게이트웨이는 오디오를 ffmpeg로 16kHz mono로 정규화하는데, **ffmpeg가 설치되어 있지 않다.** 16kHz mono WAV는 통과하고 mp3·m4a·flac·mp4·webm은 전부 `503 stt_ffmpeg_required`다. `--tool-call-parser`(§9.1)와 같은 종류의 문제이며, 같은 방식으로 — 사용자 오류가 아니라 운영자 조치로 — 보고한다.

### 12.3 동영상 분석은 불가능하다

세 가지가 모두 없다. 하나라도 있으면 가능하다.

1. **동영상 이해 모델이 카탈로그에 없다.** `LTX-2`·`Wan*`는 생성 전용이고, `qwen2.5-vl-72b`는 이미지 비전이다.
2. **클라이언트에 ffmpeg가 없다.** 프레임을 뽑아 비전 모델에 넘기는 우회로가 막힌다.
3. **게이트웨이에도 ffmpeg가 없다.** 오디오 트랙만이라도 전사하는 경로마저 막힌다(§12.2).

프레임 추출로 우회하려면 ffmpeg 의존성이 필요하고, 그것은 이 저장소가 지금까지 지켜온 "의존성 2개" 원칙을 깨는 결정이므로 임의로 하지 않았다.

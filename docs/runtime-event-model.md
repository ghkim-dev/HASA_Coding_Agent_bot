...
`cd`를 필드로 바꾼 것이 원래 실패의 절반이었다. 나머지 절반은 그 앞이다:
...
그런데 감사해 보니 **이미 옳은 자리에 닫혀 있었고 아무도 그렇게 말하지 않았을 뿐이다.** `Sandbox.writeFile`은 쓰기 전에 `mkdir(dirname, { recursive: true })`를 한다. 그래서 `create_file("a/b/c/main.py")`는 `a/b/c`를 만든다 — filesystem 호출로, 모든 플랫폼에서.

새 도구를 만들지 않았다. 한 것은 셋이다.

1. 그 동작을 invariant로 고정하고 테스트했다
2. 도구 설명과 시스템 프롬프트에 적었다

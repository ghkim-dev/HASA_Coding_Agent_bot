...
| ② | 턴마다 기록 + 영속 | **여전히 병목** — 쓰기는 `previewCli --remember` 뒤에만 있다 | 행이 없으면 저장소는 빈 파일이고 추천은 읽을 게 없다 |
| ③ | 이웃 결과로 고르기 | **함수만 있음** — `memoryEvidence`·`demoteRemembered` 는 구현·시험·변이까지 있으나 `remembered` 를 채우는 제품 호출자가 없다 | 라우터의 의미 점수는 지금 상수 `0.5` 스텁. 이게 그 자리를 채운다 |
...
| 장치 | 묻는 것 | 결과 |
|---|---|---|
| 단위 시험 | 코드가 약속대로 도는가 | **5794 / 5794** (실패 0 · 건너뜀 0) |
| 타입체크 | 세 프로젝트 | 3 / 3 |
| 변이 하네스 | 방어를 빼면 시험이 알아채는가 | **314 / 314**, 예상 밖 무반응 **0** |
| 답변 하네스 | 정답을 바꾸면 시험이 빨개지는가 | **961 / 969**, 예상 밖 **0** |
...
숫자와 상태는 **관측한 시점의 것**이다. 2026-09-12 기준 `feat/harness-auto-design`
브랜치에서 확인했고, 그날의 외부 검토가 지적한 것을 반영했다 — 가장 큰 지적은 **"함수가
존재한다"를 "제품이 쓴다"로 적지 말라**는 것이었고, 위 ②·③ 행이 그 수정이다.

다시 재려면:

```bash
node --test "src/**/*.test.ts"
pnpm design:anchors && pnpm design:mutate
pnpm design:answers
```
